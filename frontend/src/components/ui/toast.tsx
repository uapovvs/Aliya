...
const ToastContainer = () => {
  ...
  const typeColors: Record<Toast["type"], string> = {
    message: "bg-white text-gray-900 border border-gray-200",
    success: "bg-blue-700 text-white",
    warning: "bg-amber-800 text-gray-900",
    error: "bg-red-800 text-white",
  };

  const closeIconColors: Record<Toast["type"], string> = {
    message: "fill-gray-900",
    success: "fill-white",
    warning: "fill-gray-900",
    error: "fill-white",
  };
        ...
        {toasts.map((toast, index) => {
          ...
          return (
            <div
              ...
              className={clsx(
                "absolute right-0 bottom-0 shadow-lg rounded-xl leading-[21px] p-4 h-fit",
                ...
                <div className="w-full h-full flex items-center justify-between gap-4">
                  ...
                  {!toast.action && (
                    <div className="flex gap-1">
                      {toast.onUndoAction && (
                        <Button type="tertiary" svgOnly size="small"
                      ...
                      <Button type="tertiary" svgOnly size="small" onClick={() => toastStore.remove(toast.id)}>
                        <CloseIcon className={closeIconColors[toast.type]} />
                      </Button>
                    </div>
                  )}
                </div>
